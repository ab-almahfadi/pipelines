...
class GoogleAdsPipeline:
    def __init__(self):
        ...
        self.columns = [
            "conversion_action",  # From segments.conversion_action
            "conversion_action_category",  # From segments.conversion_action_category
            "all_conversions",  # From metrics.all_conversions
            "all_conversions_value",  # From metrics.all_conversions_value
            "all_conversions_by_conversion_date",  # From metrics.all_conversions_by_conversion_date
            "all_conversions_value_by_conversion_date",  # From metrics.all_conversions_value_by_conversion_date
            "click_type",  # From segments.click_type
            "ad_group_id",  # From ad_group.id
            "ad_group_name",  # From ad_group.name
            "campaign_id",  # From campaign.id
            "customer_id",  # From customer.id
            "customer_resource_name",  # From customer.resource_name
            "campaign_name",  # From campaign.name
            "date"
        ]
    ...
    def create_big_query_table_if_not_exists(self):
        ...
        try:
            schema = [
                ...
                bigquery.SchemaField("all_conversions_value", "FLOAT", mode="NULLABLE"),
                bigquery.SchemaField("all_conversions_by_conversion_date", "FLOAT", mode="NULLABLE"),
                bigquery.SchemaField("all_conversions_value_by_conversion_date", "FLOAT", mode="NULLABLE"),
                bigquery.SchemaField("click_type", "STRING", mode="NULLABLE"),
                bigquery.SchemaField("ad_group_id", "INTEGER", mode="NULLABLE"),
                ...
                bigquery.SchemaField("customer_resource_name", "STRING", mode="NULLABLE"),
        ...
        try:
            ...
            for result in results:
                try:
                    ...
                    #print(conversion_action)

                    # Prepare a row for BigQuery
                    processed_row = [
                        segments_data.get("conversionAction", "N/A"),  # conversion_action
                        segments_data.get("conversionActionCategory", "N/A"),  # conversion_action_category
                        float(metrics_data.get("allConversions", 0)),  # all_conversions
                        float(metrics_data.get("allConversionsValue", 0)),  # all_conversions_value
                        float(metrics_data.get("allConversionsByConversionDate", 0)),  # all_conversions_by_conversion_date
                        float(metrics_data.get("allConversionsValueByConversionDate", 0)),  # all_conversions_value_by_conversion_date
                        segments_data.get("clickType", "N/A"),  # click_type
                        int(ad_group_data.get("id", "N/A")),  # ad_group_id
                        ad_group_data.get("name", "N/A"),  # ad_group_name
                        int(campaign_data.get("id", "N/A")),  # campaign_id
                        int(customer_data.get("id", "N/A")),  # customer_id
                        customer_data.get("resourceName", "N/A"),  # customer_resource_name
                        campaign_data.get("name", "N/A"),  # campaign_name
    ...
    def validate_and_update_schema(self, data: List[List[Any]]):
        ...
        try:
            ...
            new_schema = {
                "conversion_action": bigquery.SchemaField(
                    "conversion_action", "STRING", mode="NULLABLE"
                ),
                "conversion_action_category": bigquery.SchemaField(
                    "conversion_action_category", "STRING", mode="NULLABLE"
                ),
                ...
                "all_conversions_by_conversion_date": bigquery.SchemaField(
                    "all_conversions_by_conversion_date", "FLOAT", mode="NULLABLE"
                ),
                "all_conversions_value_by_conversion_date": bigquery.SchemaField(
                    "all_conversions_value_by_conversion_date", "FLOAT", mode="NULLABLE"
                ),
                "click_type": bigquery.SchemaField(
                    "click_type", "STRING", mode="NULLABLE"
                ),
                ...
                "customer_resource_name": bigquery.SchemaField(
                    "customer_resource_name", "STRING", mode="NULLABLE"
                ),
                "campaign_name": bigquery.SchemaField(
                    "campaign_name", "STRING", mode="NULLABLE"
                ),
                "date": bigquery.SchemaField(
