    ...
    try:
        ...
        for i, customer_id in enumerate(customer_ids, 1):
            ...
            try:
                ...
                budget_data = await pipeline.fetch_budget_data(customer_id)
                billing_data = await pipeline.fetch_billing_data(customer_id)
                merged_data = await pipeline.merge_campaign_budget_data(customer_data, budget_data, billing_data)
                ...
                logger.info(
                    ...
                    f"(ID: {customer_id}) - Records: {len(merged_data)} - "
