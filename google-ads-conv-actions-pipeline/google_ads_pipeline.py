...
class GoogleAdsPipeline:
    def __init__(self):
        ...
        self.columns = [
            ...
            "conversion_action_primary_for_goal", 
    ...
    def create_big_query_table_if_not_exists(self):
        ...
        try:
            schema = [
                ...
                bigquery.SchemaField("conversion_action_id", "INTEGER", mode="NULLABLE"),
                bigquery.SchemaField("conversion_action_name", "STRING", mode="NULLABLE"),
                bigquery.SchemaField("conversion_action_status", "STRING", mode="NULLABLE"),
                bigquery.SchemaField("conversion_action_type", "STRING", mode="NULLABLE"),
                bigquery.SchemaField("conversion_action_primary_for_goal", "BOOLEAN", mode="NULLABLE"),
                bigquery.SchemaField("conversion_action_category", "STRING", mode="NULLABLE"),
                bigquery.SchemaField("date", "DATE", mode="NULLABLE"),
                bigquery.SchemaField(
                    "all_conversions", "FLOAT64", mode="NULLABLE"
                ),
                bigquery.SchemaField(
    ...
    async def fetch_conversion_action_data(self, customer_id: str) -> List[List[Any]]:
        """Fetch campaign data for a specific customer and prepare it for BigQuery appending."""
        try:
            self.metrics.api_calls += 1

            # Fetch data from the API
            api_response = self.ads_api.get_conversion_actions_data(
            ...
            for result in results:
                try:
                    ...
                    conversion_action_data = result.get("conversionAction", {})
                    ...
                    processed_row = [
                        int(customer_data.get("id", 0)),  # Customer ID
                        customer_data.get("descriptiveName", "id"),  # Customer Name
                        int(conversion_action_data.get("id", 0)),
                        conversion_action_data.get("name", "id"),  # Action Name
                        conversion_action_data.get("status", "N/A"),  
                        conversion_action_data.get("type", "N/A"),  
                        conversion_action_data.get("primaryForGoal", False),  
                        conversion_action_data.get("category", "N/A"),  
                        float(metrics_data.get("allConversions", 0)),  # All Conversions
                        float(metrics_data.get("allConversionsValue", 0)),  # All Conversions Value
                        segments_data.get("date", "N/A"), 
    ...
    def validate_and_update_schema(self, data: List[List[Any]]):
        ...
        try:
            ...
            new_schema = {
                ...
                "conversion_action_id": bigquery.SchemaField(
                    "conversion_action_id", "INTEGER", mode="NULLABLE"
                ),
                "conversion_action_name": bigquery.SchemaField(
                    "conversion_action_name", "STRING", mode="NULLABLE"
                ),
                "conversion_action_status": bigquery.SchemaField(
                    "conversion_action_status", "STRING", mode="NULLABLE"
                ),
                "conversion_action_type": bigquery.SchemaField(
                    "conversion_action_type", "STRING", mode="NULLABLE"
                ),
                "conversion_action_primary_for_goal": bigquery.SchemaField(
                    "conversion_action_primary_for_goal", "BOOLEAN", mode="NULLABLE"
                ),
