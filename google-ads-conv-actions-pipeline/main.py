    ...
    try:
        ...
        for i, customer_id in enumerate(customer_ids, 1):
            ...
            try:
                ...
                conversion_action_data = await pipeline.fetch_conversion_action_data(customer_id)
                all_data.extend(conversion_action_data)
                
                customer_duration = (datetime.now() - customer_start_time).total_seconds()
                logger.info(
                    f"Successfully processed customer {i}/{total_customers} "
                    f"(ID: {customer_id}) - Records: {len(conversion_action_data)} - "
