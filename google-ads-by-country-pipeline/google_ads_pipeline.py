...
class GoogleAdsPipeline:
        ...
        try:
            schema = [
                ...
                bigquery.SchemaField(
                    "country_criterion_id", "INTEGER", mode="NULLABLE"
                ),
                ...
                bigquery.SchemaField(
                    "all_conversions_value", "FLOAT64", mode="NULLABLE"
                ),
        ...
        try:
            ...
            for result in results:
                try:
                    ...
                    geo_data = result.get("geographicView", {})
                    # print(customer_data)
                    ...
                    processed_row = [
                        ...
                        geo_data.get("locationType", "N/A"),
                        int(geo_data.get("countryCriterionId", "N/A")),
                        ...
                        campaign_data.get("status", "N/A"),  # Campaign Status
                        segments_data.get("device", "N/A"),  # Segment Device Changed to device to test to avoid the big number of
    ...
    async def fetch_country_data(self, customer_id: str) -> List[List[Any]]:
        """Fetch country data for a specific customer and prepare it for BigQuery appending."""
        try:
            self.metrics.api_calls += 1

            # Fetch budget data from the API
            api_response = self.ads_api.get_campaign_country_name_data(
                customer_id=customer_id,
                start_date=self.start_date,
                end_date=self.end_date,
            )
            ...
            for result in results:
                try:
                    # Extract data with defaults for missing keys
                    country_name_data = result.get("geoTargetConstant", {})

                    # Prepare a row for BigQuery
                    processed_row = [
                        int(country_name_data.get("id", "N/A")),
                        country_name_data.get(
                            "resourceName", "N/A"
                        ),  
                        country_name_data.get("countryCode", "N/A"),
                        country_name_data.get("name", "N/A"),
                        country_name_data.get("targetType", "N/A"),
                        country_name_data.get("status", "N/A"),
                        country_name_data.get("canonicalName", "N/A"),
                    ...
                    logger.warning(
                        f"Failed to process a result for country data for customer {customer_id}: {row_ex}"
                    )
                    continue  # Skip problematic row and proceed
            ...
            self.metrics.records_processed += len(processed_data)
            logger.info(
                f"Processed {len(processed_data)} country records for customer {customer_id}"
            )

            return processed_data

        except Exception as ex:
            logger.error(f"Error fetching country data for customer {customer_id}: {ex}")
            self.metrics.failed_api_calls += 1
            self.metrics.errors.append(str(ex))
            raise
    ...
    @staticmethod
    async def merge_campaign_country_data(
        campaign_data: List[List[Any]], 
        country_data: List[List[Any]]
        # ,billing_data: List[List[Any]]
        ) -> List[List[Any]]:
        ...
        country_dict = {
            (entry[0]): entry  # (campaign_id, customer_id, date) -> budget entry
        ...
        for campaign in campaign_data:
            ...
            # Prepare merged row
            merged_row = campaign[:19]  # Start with campaign data

            if country_entry:
                # Append budget-specific data to the merged row (only for the first occurrence)
                merged_row.extend(
                    [
                        country_entry[3],  # country resource name
                    ]
                )
            else:
                # Append placeholders if no country match is found or it's a duplicate
                merged_row.extend(
                    [
                        "N/A",  # country resource name
                    ]
                )
    ...
    def validate_and_update_schema(self, data: List[List[Any]]):
        ...
        try:
            ...
            new_schema = {
                ...
                "location_type": bigquery.SchemaField(
                    "location_type", "STRING", mode="NULLABLE"
                ),
                "country_criterion_id": bigquery.SchemaField(
                    "country_criterion_id", "INTEGER", mode="NULLABLE"
                ),
                ...
                "campaign_status": bigquery.SchemaField("campaign_status", "STRING", mode="NULLABLE"),
                "device": bigquery.SchemaField("device", "STRING", mode="NULLABLE"),
                "country": bigquery.SchemaField("country", "STRING", mode="NULLABLE"),
