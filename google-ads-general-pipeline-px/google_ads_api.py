...
class GoogleAdsAPI:
    ...
    def search(
        self, customer_id: str, query: str, page_size: int = 200000
    ) -> Dict[str, Any]:
    ...
    def get_campaign_performance(
        ...
        try:
            response = self.search(customer_id, query)
            # Check if response contains results
            #print(response)
            # if 'results' in response:
            # logger.debug(f"Full response: {response}")
            return response
            # logger.warning("No campaign performance results found")
            # return []
        except Exception as e:
            logger.error(
                f"Failed to get campaign performance for customer {customer_id}: {str(e)}"
            )
            raise
        
    def get_campaign_budget_data(
        self, customer_id: str, start_date: str, end_date: str
        ) -> List[Dict[str, Any]]:
        """Fetch campaign budget data."""
        query = f"""
            SELECT 
                campaign.id, campaign.name, campaign.primary_status, 
                campaign.primary_status_reasons, campaign_budget.name, 
                campaign_budget.id, campaign_budget.period, 
                campaign_budget.status, segments.date, 
                campaign_budget.total_amount_micros, 
                campaign_budget.amount_micros, customer.id, 
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch campaign budget data for customer {customer_id}: {str(e)}"
            )
            raise
